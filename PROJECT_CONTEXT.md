...
2026-07-20 用户正式选择方案 3。项目已用确定性 SVG 路径重绘正式母版 `assets/logo.svg`，保留光标、三行菜单和青色闪光，圆角外部透明。README 使用由母版导出的 512 px `assets/logo.png`；App Icon 不再错误地让全部槽位复用同一张伪 PNG/JPEG 文件，而是提供 16、32、64、128、256、512、1024 px 对应资源。完整规范记录在 `Docs/LOGO_DESIGN.md`。验证已确认 10 个 Asset Catalog 槽位的文件、像素、RGBA 和透明圆角正确，`iconutil` 可成功合成为标准 `.icns`，16 px 至 512 px 明暗背景视觉预览可辨识；完整 24 项测试、LibreOffice 解析、性能门禁和 macOS 12 严格类型检查继续通过。

2026-07-20 用户要求将 Logo 从偏重的靛蓝改为更清新、轻盈的科技蓝。方案 3 几何造型、扁平风格和无渐变规则保持不变，新色板确定为主背景 `#2F80ED`、冰白前景 `#F7FBFF`、浅青强调 `#35D6E8`；SVG 母版、README PNG、App Icon 全尺寸资源和设计规范已同步更新。验证确认 16–1024 px 的 10 个图标槽位尺寸与 Alpha 通道正确，可合成并从最终 App 的 `AppIcon.icns` 完整反向解包；24 项核心测试、LibreOffice 模板解析、性能门禁、源码类型检查、签名、ZIP 和 DMG 挂载校验均通过。

2026-07-20 用户确认将科技蓝 Logo 更新作为补丁版本 `v1.2.1` 提交到 `hub_wample/master` 并发布 GitHub Release。Xcode 工程、本地构建脚本、最终 App、Finder 扩展和 DMG 内嵌 App 的版本均已验证为 `1.2.1`；标准发布资产为 `dist/SwiftMenu_1.2.1-arm64.dmg` 与 `dist/SwiftMenu_1.2.1-arm64.zip`，DMG SHA-256 为 `9cc847b533bab21df271a673eeda8da451482999bde35ed542bbe2ef5117675c`，ZIP SHA-256 为 `58b2b2ca9a0a635a8bb63def6a40fcb23e585bd2b4d3343a2e9695fee1fec014`。发布资产继续为 Apple Silicon（ARM64）、Apple Development 签名且未公证，Release 说明需注明作者尚未申请 Apple Developer Program 及 README Gatekeeper 单应用放行入口。

2026-07-20 根据用户截图反馈，常规页应用信息栏与标签栏下方分隔线距离过近，初版先对品牌栏增加 10 pt 顶部内边距。随后用户要求重新审视三个标签页并尽量与“关于”页保持一致。三页同尺寸截图审计发现常规、关于、菜单排序的顶部起点约为 10、54、68 pt，根因是根视图按自身理想高度居中以及菜单排序标题额外 padding。最终改为三个页面共用 `settingsPageLayout()`：通过 `GeometryReader` 强制页面容器等宽等高并顶部左对齐，统一水平 16 pt、顶部 20 pt、底部 16 pt；移除常规品牌栏和菜单排序标题的私有顶部补丁，同时将常规页根级区块间距由 24 pt 收紧至 16 pt，确保下移后底部按钮完整可见。三页 900 × 450 pt 实机截图与前后组合对比均通过，视觉 QA 见根目录 `design-qa.md`，设计取舍见 `Docs/plans/2026-07-20-settings-tabs-layout-consistency-design.md`。

2026-07-20 三标签布局统一后的完整本地构建通过：24 项核心逻辑、LibreOffice OOXML 解析、菜单性能门禁、空闲资源基准、macOS 12 类型检查、Release 参数链接检查、App/Finder 扩展签名验证以及 ZIP/DMG 验收全部成功。首次本地产物为 `build/SwiftMenu-dev.app`、`build/SwiftMenu-dev-arm64.zip`、`build/SwiftMenu-dev-arm64.dmg`；ZIP SHA-256 为 `988fe53b91dcfa47095c7d1e0a1c0120243eaaaec17045047961382f0537ede5`，DMG SHA-256 为 `ea9b3362e1c7195f1bcc196edb3bec5e0b2f275f805795a2bba143b49843855b`。

2026-07-21 用户明确要求不提升版本号，将设置页布局一致性优化提交到 `hub_wample/master`，并用新提交重新指向和替换现有 `v1.2.1` Release。替换用附件保持原名 `SwiftMenu_1.2.1-arm64.dmg` 与 `SwiftMenu_1.2.1-arm64.zip`；DMG SHA-256 为 `91bf27623392b4cba714a254ebaa121bd2f8cd81d89d273b8873f18a40e88cd4`，ZIP SHA-256 为 `a405f38afed153637ca7956a3895f7a5a9a253dbd828f7fca657fbaa2a2b829f`。旧附件已备份在忽略目录 `dist/previous/v1.2.1-before-replace.bSFc4R/`。

2026-07-21 用户反馈设置窗口约 900 × 450 pt 时两侧留白过大、整体过扁，并确认继续采用正方形方案。首轮实测内容区 500 × 470 pt 会形成约 500 × 522 pt 的最终窗口，确认系统标题栏实际占用约 52 pt；因此最终将内容区校准为 500 × 448 pt，使窗口约为 500 × 500 pt。通过 `NSViewRepresentable` 在 macOS 12 上约束宿主窗口最小/最大尺寸、禁用缩放，并在从旧宽屏恢复状态首次收缩时居中；菜单排序列表固定为容纳六项的高度，避免增高窗口产生空白条纹行。三个标签页内容与交互保持不变，设计记录见 `Docs/plans/2026-07-21-settings-window-proportion-design.md`。

2026-07-21 正方形窗口最终视觉 QA 通过：常规、菜单排序、关于三页均实测为 500 × 500 pt；第一轮空白条纹行和第二轮列表滚动条均已修复，根目录 `design-qa.md` 为 `final result: passed`。最后一处列表高度修复进入正式产物后，完整构建再次通过 24 项核心逻辑、LibreOffice OOXML 解析、菜单性能门禁、空闲资源基准、macOS 12 类型检查、Release 参数链接检查及 App/Finder 扩展/ZIP/DMG 签名验收。用户明确要求不提升版本号，将本次正方形窗口与 README 截图修改提交到 `hub_wample/master`，并再次替换现有 `v1.2.1` Release；正式同名附件 DMG SHA-256 为 `fa84fdac657d79df6e647cee2b5d1b11bf9c99baa6ae4eb98444bd25f6ba6895`，ZIP SHA-256 为 `d4f8da4280425ffcdbb40c11ebf39a17f6621777684441d3a504fe420181693d`，上一组正式附件已保存在忽略目录 `dist/previous/v1.2.1-before-square-window-replace.oT50x9/`。

2026-07-21 用户提供正方形窗口下的“常规”和“菜单排序”最终截图，用于替换 README 的“自定义设置中心”和“自定义菜单调整顺序”展示图。资源路径保持为 `assets/screenshot_settings.png` 与 `assets/screenshot_menu_sort.png`，因此中文和英文 README 无需修改引用；两张新图均为 1000 × 1000 px，README 继续按 300 px 宽度并排展示。
...
2026-07-20 根据用户要求，中文和英文 README 均加入未公证测试版首次打开步骤：先尝试启动，再前往“系统设置 → 隐私与安全性 → 安全性”点击“仍要打开”，或由熟悉终端的用户仅对 `/Applications/SwiftMenu.app` 执行 `xattr -dr com.apple.quarantine`。文档明确禁止用 `sudo spctl --master-disable` 全局关闭 Gatekeeper，并提醒只有在来源和校验值可信时才应放行。Finder 扩展启用路径同步更新为当前 macOS 的“通用 → 登录项与扩展”，同时保留旧版 macOS 的“系统偏好设置 → 扩展”说明。

2026-07-20 用户确认直接发布当前 `v1.2.0`。由于作者尚未申请 Apple Developer Program，本次发布资产为 Apple Development 签名、未公证的 ARM64 DMG/ZIP；GitHub Release 说明必须如实写明该原因，并引导用户按照 README 的单应用 Gatekeeper 放行步骤打开，不能声称已经 Developer ID 签名或 Apple 公证。
